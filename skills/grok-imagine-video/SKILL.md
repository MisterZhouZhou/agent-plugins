...
export XAI_API_BASE_URL="https://your-api-host/v1"

# Optional: override the default model names
export XAI_IMAGE_MODEL="grok-imagine-image"
export XAI_VIDEO_MODEL="grok-imagine-video"
```
...
export XAI_API_BASE_URL="https://your-api-host/v1"  # optional
export XAI_IMAGE_MODEL="grok-imagine-image"         # optional
export XAI_VIDEO_MODEL="grok-imagine-video"         # optional
```

For OpenClaw integration, add to workspace `.env` or manage via gateway config.

If `XAI_API_BASE_URL` is not set, the skill uses the default xAI endpoint: `https://api.x.ai/v1`.
If `XAI_IMAGE_MODEL` or `XAI_VIDEO_MODEL` is not set, the skill uses xAI defaults.
