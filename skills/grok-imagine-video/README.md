...
If `XAI_API_BASE_URL` is not set, the client defaults to `https://api.x.ai/v1`.
If `XAI_IMAGE_MODEL` or `XAI_VIDEO_MODEL` is not set, the client uses the xAI default model names.
