...
|---|---|---|---|
| `TYPESAFE_BASE_URL` | No | API endpoint URL | `https://opencode.ai/zen/v1/systemone` |
| `TYPESAFE_API_KEY` | No | API authentication key | `public` (anonymous) |
| `TYPESAFE_DEFAULT_MODEL` | No | Default model to use | `jev-1.13-free` |

### Config file format

Credentials are stored in `~/.agents/typesafe-ai-third/config.json`:
...
For the default OpenCode Zen free tier (no custom endpoint):
...
Environment variables always override the config file. Do not commit the config file.

### Interactive configuration

```bash
# Run interactive setup
python skills/typesafe-ai-third/bin/systemone-call --configure

# Show current configuration (key masked)
python skills/typesafe-ai-third/bin/systemone-call --show-config
```

The script prompts for missing values, hides API-key input, and persists them to
`~/.agents/typesafe-ai-third/config.json` after confirmation.

### Quick start

```bash
# Using defaults (OpenCode Zen free tier)
echo '{"questions":{"my_q":{"type":"noul","question":"Is this positive?","instructions":"yes/no"}}}' \
  | python skills/typesafe-ai-third/bin/systemone-call

# Using environment variables
TYPESAFE_API_KEY=your-key \
  echo '{"questions":{"my_q":{"type":"noul","question":"Is this positive?"}}}' \
  | python skills/typesafe-ai-third/bin/systemone-call

# Using config file (after --configure)
echo '{"questions":{"my_q":{"type":"noul","question":"Is this positive?"}}}' \
  | python skills/typesafe-ai-third/bin/systemone-call

# Dry run to verify configuration
echo '{"questions":{"test":{"type":"noul","question":"test?"}}}' \
  | python skills/typesafe-ai-third/bin/systemone-call --dry-run
```

### Troubleshooting

- `HTTP 401`: API key is invalid or missing. Run `--configure` or set `TYPESAFE_API_KEY`.
- `HTTP 404`: Check that `TYPESAFE_BASE_URL` ends with `/v1/systemone` or let the script auto-normalize.
- Connection timeout: Check network access to the endpoint; the script retries with exponential backoff.
- Missing credentials in non-interactive run: Set environment variables or run `--configure` in a terminal.