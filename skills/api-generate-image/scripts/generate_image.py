...
def write_config(values: dict[str, str], path: Path = CONFIG_PATH) -> None:
    ...
    try:
        ...
        pass
    print(f"Saved api-generate-image config to {path}", file=sys.stderr)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate images through an OpenAI-compatible Image API")
    ...
    started = time.time()
    is_official = base_url.rstrip("/") == "https://api.openai.com/v1"
    # Some third-party proxies block requests with the OpenAI SDK User-Agent.
    # Override it when targeting a custom endpoint.
    extra_headers = {} if is_official else {"User-Agent": "python-httpx/0.27.0"}
    ...
    if is_official:
        # quality and output_format are gpt-image-2 specific; keep them for the
        # official endpoint only as a safety measure for older proxies.
        generate_kwargs["quality"] = args.quality
        generate_kwargs["output_format"] = args.output_format
